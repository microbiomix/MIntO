...
##########################################
# Handle nanopore-only assemblies
##########################################
...
assemblies['nanopore']                 = nano_samples
...
def vamb_binning_preparation_output():
    # No Illumina samples
    if not ilmn_samples:
        return([])

    # Illumina is not needed
    if not any(x in SCAFFOLDS_type for x in ["illumina_single", "illumina_coas", "illumina_single_nanopore"]):
        return([])
...
def graphmb_binning_preparation_output():
    ...
    return(expand("{wd}/{omics}/8-1-binning/scaffolds_nanopore.{min_length}/graphmb/{nanopore}/{assembly_preset}/assembly_depth.txt",
                  ...
                  nanopore = nano_samples,
                  assembly_preset = METAFLYE_presets.keys()))

rule all:
    input:
        vamb_binning_preparation_output(),
        graphmb_binning_preparation_output(),
...
if len(ilmn_samples) > 0:
    ...
    checkpoint make_assembly_batches:
        ...
        run:
            ...
            batch_dir = os.path.dirname(output.done)
            os.makedirs(batch_dir, exist_ok=True)
            for filename in os.listdir(batch_dir):
                if filename.startswith("batch") and filename.endswith(".list"):
            ...
            with open(str(log), 'w') as f:
                ...
                for i in range(0, len(input.assemblies)):
                    ...
                    if (i and ((current_batchfilesize + current_assemblysize) > params.batch_filesize)):
                        ...
                        batch_list  = batch_dir + f"/batch{batch}.list"
...
if len(nano_samples) > 0:
    ...
    rule graphmb_depth_coverm:
        ...
        shadow:
            "minimal"
        log:
            "{wd}/logs/{omics}/8-1-binning/scaffolds_nanopore.{min_length}/graphmb/{nanopore}/{assembly_preset}/assembly_depth.log"
        threads:
        ...
        resources:
            ...
            sort_threads = 3,
            sort_mem = 10,
            mem = lambda wildcards, input, attempt: 25 + 20*(attempt-1)
        conda:
            minto_dir + "/envs/MIntO_base.yml"
        shell:
            """
            ...
            time (
                minimap2 -t {resources.map_threads} -a -x map-ont {input.fasta} {input.ont} \
                    | samtools sort --threads {resources.sort_threads} -m {resources.sort_mem}G - \
                    > graphmb_edges.bam
                samtools index graphmb_edges.bam
                coverm contig --methods metabat --min-read-percent-identity 90 --threads {resources.coverm_threads} --output-file assembly_depth.txt --bam-files graphmb_edges.bam
                rsync -a assembly_depth.txt {output.depth}
            ) >& {log}
            """
...
#  GraphMB graph-edge mode for ONT-only MetaFlye assemblies: graphmb
...
# GraphMB will run for every NANOPORE_SAMPLES x METAFLYE_presets combination
# when graphmb is included in BINNERS.
NANOPORE_SAMPLES:
